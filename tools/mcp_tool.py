    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            async with server._rpc_lock, _track_inflight_rpc(
                ...
                try:
                    # Forward opt-in request metadata and retain the
                    # stdio fast-fail/reconnect behavior. Both concerns must
                    # be applied to the same MCP call; using the fast-fail
                    # path must not silently drop ``meta``.
                    ...
                    if not _watch_ok:
                        # If the RPC is a test stub or otherwise not awaitable,
                        # close the watcher coroutine created for the probe so
                        # it does not leak a RuntimeWarning.
                        if inspect.iscoroutine(_watch_coro):
                            _watch_coro.close()
